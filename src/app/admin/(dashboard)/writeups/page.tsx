...
export default function ContentAdminPage() {
  ...
  const handleEdit = (item: ContentItem) => {
    setId(item.id);
    setCategory(item.category || "Cyber");
    setTitle(item.title);
  ...
  const handleGithubImport = async () => {
    if (!githubSyncUrl) {
      setError("Please enter a GitHub file link first (e.g. https://github.com/user/repo/blob/main/README.md)");
      return;
    }

    // Convert github.com to raw.githubusercontent.com safely
    let rawUrl = githubSyncUrl;
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to fetch. Ensure it is a public repository and points to a file.");
      ...
      if (titleMatch && titleMatch[1]) {
        ...
        setTitle("Imported GitHub Writeup");
      }

      // 2. Remove the matched title to avoid duplicate H1 in the content editor
      let cleanContent = markdown;
      if (titleMatch) {
         cleanContent = cleanContent.replace(titleMatch[0], "").trim();
      }

      // 3. Summary (first non-empty paragraph that isn't a heading/image)
      const paragraphs = cleanContent.split("\n\n");
      const summaryText = paragraphs.find(p => p.trim() && !p.startsWith("#") && !p.startsWith("!") && !p.startsWith("["));
      if (summaryText && !summary) {
        // limit to 200 chars without cutting words brutally if possible
        setSummary(summaryText.substring(0, 150) + "...");
      }

      // 4. Set Content
      setContent(cleanContent);
      setSuccess("Successfully fetched and parsed Markdown from GitHub! You can now edit the content below.");
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 font-mono">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Write-ups</h1>
          <p className="text-[10px] text-text-muted mt-1 uppercase tracking-wider">
            Manage cybersecurity articles and devlogs
          </p>
        </div>
        {!formOpen && (
      ...
      {formOpen && (
        <div className="glass-card p-6 border border-white/5 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* GitHub Link */}
            <div className="space-y-2">
              <label className="block text-xs text-text-muted uppercase">GitHub Repository/File Link</label>
              <div className="flex gap-2">
                <input
                  type="url"
                  value={githubSyncUrl}
                  onChange={(e) => setGithubSyncUrl(e.target.value)}
                  placeholder="https://github.com/adrianfahrezi404/repo/blob/main/README.md"
                  className="w-full bg-bg-secondary border border-white/10 rounded-lg px-4 py-2.5 text-xs text-text-primary focus:border-accent-cyan/50 focus:outline-none"
                />
                <button
                  type="button"
                  onClick={handleGithubImport}
                  disabled={importing || !githubSyncUrl}
                  className="px-4 py-2 bg-accent-purple/20 text-accent-purple border border-accent-purple/30 font-bold text-xs rounded-lg hover:bg-accent-purple/30 disabled:opacity-50 whitespace-nowrap transition-all"
                >
                  {importing ? "Fetching..." : "Import .md"}
                </button>
              </div>
              <p className="text-[10px] text-text-muted">Paste a GitHub file link and click Import to auto-fill the content below.</p>
            </div>
